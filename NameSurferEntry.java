...
public class NameSurferEntry implements NameSurferConstants {
	...
	public NameSurferEntry(String line) {
		...
		for (int i = 0; i < line.length(); i++) {
			if (line.charAt(i) == ' ') {
				// gives name the value of a substring before the first space
				name = line.substring(0, i);

				// substring after the first space contains integers
				integers = line.substring(i + 1);
				break;
			}
		}
		StringTokenizer tokenizer = new StringTokenizer(integers);
		while (tokenizer.hasMoreTokens()) {
			// each integer is added in the arrayList ranks
			ranks.add(Integer.parseInt(tokenizer.nextToken()));
		}
	}
